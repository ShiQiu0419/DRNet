    ...
    for i in range(NUM_REPEAT):
        ...
        for _, data in enumerate(test_dataloader, 0):
            ...
            for v in range(NUM_VOTE):
                ...
                # temp_pred, error1, error2, error3, error4, _ = model(new_points, batch_one_hot_cls)
                temp_pred = model(new_points, batch_one_hot_cls)
                pred += F.softmax(temp_pred, dim = 2)
            pred /= NUM_VOTE
        ...
        if np.mean(instance_ious) > global_Inst_mIoU:
